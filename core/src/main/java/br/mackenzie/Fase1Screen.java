    ...
    public class Fase1Screen extends FaseBaseScreen{
        ...
        private float maxGameTime = 100f;  // 18 minutos é o tempo dessa partida
        ...
        private final float MAX_PROGRESS_TIME = 62.45f;
        ...
        public void show() {
            ...
            vitoriaSound = Gdx.audio.newSound(Gdx.files.internal("Sons_Fase1/SomTorcida.mp3"));
            torcidaSound = Gdx.audio.newSound(Gdx.files.internal("Sons_Fase1/SomTorcida.mp3"));
            // Pause
        ...
        public void render(float dt) {
            ...
            switch (gameState) {
                case RUNNING:
                    ...
                    // Calculo do tempo do jogador e da fase
                    gameTime+=dt;

                    if (gameTime >= 1f){
                        currentGameTime++;
                        gameTime = 0;   // Reiniciando a contagem

                        System.out.println(currentGameTime);
                    }
                    
                    // Parar o jogo se chegar no limite de tempo
                    if (currentGameTime >= maxGameTime && !gameOver){
                        gameOver = true;
        ...
        private void updateGameObjects(float dt) {
            ...
            final float porcentagem_final = 0.94f;
            ...
            if (esperandoCicloParalaxe) {
                // A condição de troca é quando o fundo estiver voltando para a posição 0.
                // O seu paralaxe funciona com base em (faseAtual.backgroundOffsetX % worldWidth).
                // Queremos trocar quando o 'offset' estiver próximo de um múltiplo inteiro de worldWidth.
                ...
                // Se o offset for muito pequeno (próximo de zero ou worldWidth)
                // Usamos um pequeno Epsilon (ex: 0.1f) para tolerância de ponto flutuante.
                if (distanceIntoCurrentCycle < 0.1f) {
                    ...
                    esperandoCicloParalaxe = false; // Troca concluída!

                    faseAtual.setParalaxVelocity(0.0f);

                    movimentoFinalPlayer = true;

                    // Importante: Você pode precisar ajustar o backgroundOffsetX da FASE ATUAL
                    // para que a nova imagem comece perfeitamente do zero.
                    faseAtual.backgroundOffsetX = 0; // Se a nova imagem deve começar do zero
                }
            }
        ...
        private void drawUI() {
            ...
            // Desenho do contador do tempo da partida
            String tempoPartida = String.format("Tempo: %d / %.0f", currentGameTime, maxGameTime);
            fonte.getData().setScale(2f);
            fonte.setColor(1, 1, 0, 1); // Cor amarela do texto

            // Centralizado no topo
            float textoX = uiStage.getViewport().getWorldWidth() / 2f;
            float textoY = uiStage.getViewport().getWorldHeight() - 80f;
            fonte.draw(spriteBatch, tempoPartida, textoX, textoY, 0, Align.center, false);
